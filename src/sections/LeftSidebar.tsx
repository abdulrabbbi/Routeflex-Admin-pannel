...
const menuItems = [
   { icon: MdDashboard, label: "Dashboard", active: true, path: '/' },
...
const LeftSidebar = ({ isOpen, setIsOpen }: any) => {
   return (
      <>
         ...
         <div
            className={`
        fixed lg:static inset-y-0 left-0 w-64 bg-white transform 
         ...
         >
            ...
            <nav className="flex-1 px-3 py-4 space-y-1">
               {menuItems.map((item) => (
                ...
                ${item.active ? "bg-[#24123A0D] text-gray-900" : "text-gray-600 hover:bg-gray-50 hover:text-gray-900"}
              `}
                  >
