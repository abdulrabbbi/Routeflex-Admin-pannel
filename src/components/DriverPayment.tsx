           ...
           <tbody className="divide-y divide-gray-100">
             {drivers.map((driver, index) => (
               <tr key={index} className={index % 2 === 0 ? "bg-[#f8fafc]" : "bg-white"}>
                 <td className="px-6 py-4 text-sm text-[#1e1e38]">{driver.id}</td>
                 ...
                 <td className="px-6 py-4">
                   <span
                     className={`inline-flex w-2 h-2 rounded-full ${
                       driver.paymentStatus === "Pending" ? "bg-red-500" : "bg-[#22c55e]"
                     }`}
                   />
                   <span className="ml-2 text-sm">{driver.paymentStatus}</span>
                 </td>
                 <td className="px-6 py-4">
