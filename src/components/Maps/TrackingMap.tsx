...
const vehicleIcon = new L.Icon({
  iconUrl: Images.MapCar, // Ensure this file exists
  iconSize: [32, 32],
  iconAnchor: [16, 16],
});
...
const Map = ({ route, currentLocation }: MapProps) => {
  const center = [currentLocation.lat, currentLocation.lng] as [number, number];
  ...
  return (
    <div className="relative rounded-xl overflow-hidden shadow-sm border border-gray-200">
      ...
      <MapContainer
        center={center}
        zoom={14}
        className="h-[500px] w-full" // Ensure the map has explicit height & width
        zoomControl={false}
      >
        <TileLayer
